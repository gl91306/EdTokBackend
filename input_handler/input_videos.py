...
video_path = "input_handler/Download.mp4" # replace this with path to the raw video that is recieved as input from frontend
...
def extract_vid_and_audio(video_path, audio_path, temp_vid_path):
    ...
    # write the two clips as seperate files
...
downsampled_y = np.stack(
    [downsample_channel(ycbcr_video[i][:, :, 0]) for i in range(frames)])
downsampled_cb = np.stack(
    [downsample_channel(ycbcr_video[i][:, :, 1]) for i in range(frames)])
downsampled_cr = np.stack(
    [downsample_channel(ycbcr_video[i][:, :, 2]) for i in range(frames)])

downsampled_ycbcr = np.dstack(
    (downsampled_y, downsampled_cb, downsampled_cr), axis=-1)


rgb_video_array = np.empty([frames, (height//4)*4, (width//4)*4, channel])

for i in range(video.shape[0]):
    ycbcr_video[i] = cv2.cvtColor(downsampled_ycbcr[i], cv2.COLOR_YCrCb2RGB)
...
os.remove(temp_vid_path) # delete original video and uncompressed video to save space
