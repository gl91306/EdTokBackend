...
video_path = "./EdTokBackend/video_handler/Download.mp4"
...
downsampled_y = np.stack(
    [downsample_channel(ycbcr_video[i][:, :, 0]) for i in range(frames.shape[0])])
downsampled_cb = np.stack(
    [downsample_channel(ycbcr_video[i][:, :, 1]) for i in range(frames.shape[0])])
downsampled_cr = np.stack(
    [downsample_channel(ycbcr_video[i][:, :, 2]) for i in range(frames.shape[0])])
